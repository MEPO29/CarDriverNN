...
selectedCars = []
selected = 0
lines = True  # Si es True, se muestran las líneas de los sensores del coche
player = True  # Si es True, se muestra el coche del jugador
display_info = True  # Si es True, se muestra la información en pantalla
frames = 0
maxSpeed = 10
number_track = 1

# Carga de imágenes de los coches
...
# Definición de la red neuronal usando Keras
...
# Clase del coche utilizando el modelo de Keras
class Car:
    def __init__(self, input_size, hidden_size, output_size):
        self.model = create_model(input_size, hidden_size, output_size)
        self.score = 0
        self.c1 = 0, 0
        self.c2 = 0, 0
        self.c3 = 0, 0
    ...
    def feedforward(self):
        inputs = np.array([[self.d1], [self.d2], [self.d3], [self.d4], [self.d5], [self.velocity]])
        inputs = inputs.T  # Keras espera la entrada como un batch, por lo que se necesita transponer
        output = self.model.predict(inputs, verbose=0)
        self.outp = output.flatten()  # Aplana la salida
        return self.outp
    ...
    def takeAction(self):
        if self.outp[0] > 0.5:  # Acelerar
            self.set_accel(0.2)
        else:
            self.set_accel(0)
        if self.outp[1] > 0.5:  # Frenar
            self.set_accel(-0.2)
        if self.outp[2] > 0.5:  # Girar a la derecha
            self.rotate(-5)
        if self.outp[3] > 0.5:  # Girar a la izquierda
            self.rotate(5)
        return


# Inicialización de la lista de coches controlados por redes neuronales
nnCars = []
num_of_nnCars = 200  # Número de coches controlados por redes neuronales
alive = num_of_nnCars  # Número de coches que no han colisionado
collidedCars = []  # Lista de coches que han colisionado
...
text6 = font.render('B - Criar', True, white)
...
text11 = font.render('M - Criar y Siguiente Pista', True, white)
...
def displayTexts():
    ...
    infoText2 = font.render('Coches: ' + str(num_of_nnCars), True, white)
...
# Inicialización de la red neuronal del coche
inputLayer = 6
hiddenLayer = 6
outputLayer = 4
...
# Creación de los coches controlados por redes neuronales
...
def redrawGameWindow():
    ...
    # Actualizar y dibujar coches controlados por redes neuronales
    for nncar in nnCars:
        if not nncar.collided:
            nncar.update()

        if nncar.collision():  # Verificar si algún coche ha colisionado
    ...
    # Actualizar y dibujar el coche del jugador
    if player:
        car.update()
        if car.collision():
            car.resetPosition()
    ...
    for event in pygame.event.get():  # Verificar eventos
        ...
        if event.type == pygame.KEYDOWN:  # Si el usuario presiona una tecla
            if event.key == ord("l"):  # Alternar visualización de líneas
                car.showLines()
                lines = not lines
            if event.key == ord("c"):  # Limpiar coches colisionados
                for nncar in nnCars:
                    if nncar.collided:
                        nnCars.remove(nncar)
                        if not nncar.yaReste:
                            alive -= 1
            if event.key == ord("a"):  # Alternar visualización del coche del jugador
                player = not player
            ...
            if event.key == ord("b"):  # Criar nuevos coches a partir de los seleccionados
            ...
            if event.key == ord("m"):  # Criar nuevos coches y generar nueva pista aleatoria
                if len(selectedCars) == 2:
                    for nncar in nnCars:
                        nncar.score = 0
